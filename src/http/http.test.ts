...
describe("ForgeHttpClient — metrics", () => {
  ...
  it("성공 응답 시 method/host/status 레이블로 counter와 histogram을 기록한다", async () => {
    ...
    const [[successHandler]] = vi.mocked(mockInstance.interceptors.response.use).mock.calls as [
      [(res: unknown) => unknown, (err: unknown) => Promise<unknown>],
    ];
    const fakeResponse = {
      status: 200,
      config: {
  ...
  it("HTTP 에러 응답 시 실제 status 코드로 메트릭을 기록하고 에러를 re-reject한다", async () => {
    ...
    const [[, errorHandler]] = vi.mocked(mockInstance.interceptors.response.use).mock.calls as [
      [(res: unknown) => unknown, (err: unknown) => Promise<unknown>],
    ];
    const fakeError = {
      config: {
