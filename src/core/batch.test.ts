...
describe("processBatch", () => {
  ...
  it("결과를 입력 배열과 같은 순서로 반환한다", async () => {
    const results = await processBatch([1, 2, 3, 4, 5], async (n) => n * 10, { chunkSize: 2 });
  ...
  it("chunkSize <= 0이면 에러를 던진다", async () => {
    await expect(processBatch([1, 2, 3], async (n) => n, { chunkSize: 0 })).rejects.toThrow(
      "chunkSize must be greater than 0",
    );

    await expect(processBatch([1, 2, 3], async (n) => n, { chunkSize: -1 })).rejects.toThrow(
      "chunkSize must be greater than 0",
    );
  });
  ...
  it("onChunkComplete가 청크마다 올바른 processed/total로 호출된다", async () => {
    ...
    await processBatch([1, 2, 3, 4, 5], async (n) => n, {
      chunkSize: 2,
  ...
  it("청크 사이에 delayMs만큼 대기한다", async () => {
    ...
    let resolved = false;
    const p = processBatch([1, 2, 3, 4], async (n) => n, { chunkSize: 2, delayMs: 500 }).then(
      (r) => {
        resolved = true;
        return r;
      },
  ...
  it("fn이 실패하면 즉시 throw한다", async () => {
    ...
    await expect(processBatch([1, 2, 3, 4, 5], fn, { chunkSize: 5 })).rejects.toThrow(
      "failed at 3",
    );
  });
});
