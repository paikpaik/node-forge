...
describe('MetricsInterceptor', () => {
  ...
  it('예외 발생 시 에러 status로 기록하고 에러를 재-throw한다', async () => {
    ...
    await expect(lastValueFrom(interceptor.intercept(ctx, makeErrorHandler(err)))).rejects.toThrow('Not Found')
  ...
  it('status 없는 예외는 status 500으로 기록한다', async () => {
    ...
    await expect(lastValueFrom(interceptor.intercept(ctx, makeErrorHandler(err)))).rejects.toThrow('Unexpected')
