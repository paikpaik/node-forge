...
function mockRequest(overrides: { method?: string; routeOptionsUrl?: string | undefined; startTime?: number } = {}): FastifyRequest {
  return {
...
describe('fastifyMetrics', () => {
  ...
  it('httpMetrics: true이면 onResponse 훅이 counter와 histogram을 기록한다', async () => {
    ...
    const req = mockRequest({ method: 'POST', routeOptionsUrl: '/orders', startTime: Date.now() - 50 })
