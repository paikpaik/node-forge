...
describe('ForgeRedisClient', () => {
  ...
  describe('cGet', () => {
    ...
    it('compareKey가 없으면 cachedAt=0 기준으로 비교해 데이터를 반환한다', async () => {
      mockClient.mget.mockResolvedValue([
        JSON.stringify({ cachedAt: 500, data: 'value' }),
        null,
      ])
  ...
  describe('mset', () => {
    it('여러 키-값을 pipeline으로 직렬화해 저장한다', async () => {
      ...
      expect(mockPipeline.set).toHaveBeenCalledWith('user:1', expect.stringContaining('"cachedAt"'))
      expect(mockPipeline.set).toHaveBeenCalledWith('user:2', expect.stringContaining('"cachedAt"'))
  ...
  describe('zadd', () => {
    it('score-member 쌍을 평탄화해서 zadd를 호출한다', async () => {
      ...
      expect(await client.zadd('rank', [{ score: 100, member: 'user:1' }, { score: 200, member: 'user:2' }])).toBe(2)
  ...
  describe('getRankAndScore', () => {
    it('rank(1-based)와 score를 함께 반환한다', async () => {
      const mockPipeline = {
        ...
        exec: vi.fn().mockResolvedValue([[null, 0], [null, '1500']]),
      }
      mockClient.pipeline.mockReturnValue(mockPipeline)
  ...
  describe('subscribe', () => {
    ...
    it('message 이벤트 수신 시 핸들러를 JSON 역직렬화해서 호출한다', () => {
      ...
      const mockSubscriber = {
        subscribe: vi.fn().mockResolvedValue(undefined),
        on: vi.fn((event, cb) => { if (event === 'message') messageListener = cb }),
        disconnect: vi.fn(),
        unsubscribe: vi.fn().mockResolvedValue(undefined),
      }
      mockClient.duplicate.mockReturnValue(mockSubscriber)
